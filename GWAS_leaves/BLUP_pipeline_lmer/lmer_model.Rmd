...
random_effect <- c("Clone")
fixed_effects <- c("Meta.block", "Block")
trait_names <- colnames(dat)[10:ncol(dat)]
...
```{r}
stats_random_M3 <- lapply(trait_names, function(t) {
  res <- ranova(lmer_model3[[t]])
  pval <- res$`Pr(>Chisq)`[2]
  data.frame(trait = t,
             p_val = pval)
}) %>% bind_rows %>% tibble::as_tibble()
message(paste0("\tTraits with non-significant random effect: ", stats_random_M3 %>% filter(p_val > 0.05) %>% pull(trait), "\n"))
```
